...
public class ProductListingPage extends GeneralPages
{
    ...
    public boolean is()
    {
        return super.is() && matches(has(itemCount));
    }
    
    public String getCategoryType()
    {
    	String categoryType = RegExUtils.getFirstMatch(Context.getPage().getUrl().toString(), "\\/Search-Show?cgid=\\/([^/]*)", 1);
    	Assert.assertTrue("Expected category type to be contained in URL", !StringUtils.isBlank(categoryType) && categoryType.toLowerCase().contains("cgid"));
    	Assert.assertEquals("Expected category type to be contained in URL", categoryType);
    ...
    public String getCategoryId()
    {
    	...
    	String categoryId = RegExUtils.getFirstMatch(categoryType, "\\/Search-Show?cgid=\\/([^/]*)", 1);
