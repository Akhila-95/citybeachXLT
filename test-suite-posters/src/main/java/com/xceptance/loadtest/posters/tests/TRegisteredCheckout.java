...
import com.xceptance.loadtest.posters.flows.LoginFlow;
...
/**
 * Starts visit at landing page, creates a new account, adds products to the
 * cart via searching or browsing, and proceeds through checkout but abandons
 * the checkout after the payment step.
 * 
...
public class TRegisteredCheckout extends LoadTestCase {
	...
	public void test() throws Throwable {
		...
		if (Context.get().data.totalAddToCartCount > 0) {
			...
			// Follow checkout steps (do not place final order but abandon checkout after
			// payment)
