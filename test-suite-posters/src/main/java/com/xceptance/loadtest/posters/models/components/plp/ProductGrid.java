...
public class ProductGrid implements Component
{
    ...
    {
        return Page.find().byCss(".product-grid");
    }
    ...
    public HtmlElement getFilteredProductsQuick()
    {
    	
    		return getProductUrlsFromExistingTilestoQuickviewAndApplyFilter();
    ...
    private List<String> getProductUrlsFromPageStateAndApplyFilter(List<String> discardedUrls)
    {
    	...
    	for(int i=0; i<products.length(); i++)
    	{
    		...
    		try
    		{
    			productLinks.add("/on/demandware.store/Sites-CityBeachAustralia-Site/default/Product-ShowQuickView?pid=" + id);
    		}
    		catch(Exception uee)
    		{
    ...
    private List<String> getProductUrlsFromExistingTilesAndApplyFilter(List<String> discardedUrls)
    {
    	...
    	List<HtmlElement> productElements = locate().byCss(".tile-image-main-div>a[href]").all();
    ...
    private HtmlElement getProductUrlsFromExistingTilestoQuickviewAndApplyFilter()
    {
    	// Get all product tiles that contain a link
    	HtmlElement productElements = locate().byCss(".image-container>a.quickview").random();
    	// Retrieve all URLs and apply the URL filter
    ...
    {
    	// Get all product tiles that contain a link
    	List<HtmlElement> productElements = locate().byCss("div[class='product']").all();
    	
    	// Retrieve all URLs and apply the URL filter
    	return productElements;
    }
    public HtmlElement GetLoadmoreButton()
    {
    	HtmlElement getLoadmoreButton = locate().byCss(".btn.btn-outline-primary.more").first();
