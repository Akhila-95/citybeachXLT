...
import com.xceptance.loadtest.api.data.Paymentdata;
...
public class PickupInStore extends PageAction<PickupInStore>
{
	private Account account;
	private CreditCard creditCard;
	private int Cardindex;
	
	public PickupInStore()
	{
		this(Context.get().data.getAccount().get(),Context.get().data.getAccount().get().creditCards);
	}
	
	public PickupInStore(Account account,EnumConfigList<CreditCard> cards )
	{
	...
	protected void doExecute() throws Exception
	{
		// Fill shipping address form
	        ...
	        // Assert.assertEquals("Responce expected",new JSONObject(res.getContentAsString()));
			 ...
			 parms.add(new NameValuePair("dwfrm_shipping_shippingAddress_addressFields_address1", account.PickupAtStore.addressLine1));
			 parms.add(new NameValuePair("dwfrm_shipping_shippingAddress_addressFields_address2", account.PickupAtStore.addressLine2));
			 parms.add(new NameValuePair("dwfrm_shipping_shippingAddress_addressFields_city", account.PickupAtStore.city));
			 parms.add(new NameValuePair("dwfrm_shipping_shippingAddress_addressFields_states_stateCode", account.PickupAtStore.stateCode));
			 parms.add(new NameValuePair("dwfrm_shipping_shippingAddress_addressFields_postalCode", account.PickupAtStore.zip));
			 parms.add(new NameValuePair("dwfrm_shipping_shippingAddress_addressFields_country", account.PickupAtStore.countryCode));
			 parms.add(new NameValuePair("dwfrm_shipping_shippingAddress_addressFields_phone", account.mobile));
			 parms.add(new NameValuePair("dwfrm_shipping_shippingAddress_addressFields_addressType", "auspost"));
	         ...
	         WebResponse response2=req2.fire();
				 ...
				 parm.add(new NameValuePair("address1", account.PickupAtStore.addressLine1));
				 parm.add(new NameValuePair("address2", account.PickupAtStore.addressLine1));
      			 parm.add(new NameValuePair("city", account.PickupAtStore.city));
				 parm.add(new NameValuePair("postalCode", account.PickupAtStore.zip));
				 parm.add(new NameValuePair("stateCode", account.PickupAtStore.stateCode));
				 parm.add(new NameValuePair("countryCode",account.PickupAtStore.countryCode));
		         ...
		         WebResponse response=req1.fire();
			         ...
			         Paymentparms.add(new NameValuePair("dwfrm_billing_addressFields_address2", account.shippingAddress.addressLine1));
...
//			         Paymentparms.add(new NameValuePair("dwfrm_billing_creditCardFields_cardType", "visa"));
			      ...
			      // Assert.assertEquals("Responce expected",response3.getContentAsString());
...
//			                    .POST();
//			         WebResponse response6=req6.fire();
			         //Assert.assertEquals("Responce  ", response6.getContentAsString());
